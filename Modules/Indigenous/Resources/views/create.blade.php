...
@section('content')
    <el-row class="pb-2">
        <el-col :md="24">
            <el-card>
                <div slot="header"><i class="el-icon-plus text-primary"></i> Create Indigenous</div>
                <div>
                    @include('indigenous::components.form')
                </div>
            </el-card>
        </el-col>
    </el-row>
            ...
            methods: {
                submitForm(formRefs){
                    this.$refs[formRefs].validate((valid) => {
                        if (valid) {
                            axios.post('{{route('api.indigenous.store')}}', this.form)
                                .then(res => {
                                    swal('Success', 'Saved successfully!', 'success')
                                        .then(() => {
                                            window.location = '{{route('indigenous.index')}}'
                                        });
                                })
                                .catch(err => {
